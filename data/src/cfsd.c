...
#define LOCKFILE VARLIBDIR "/.cfsd.lockfile"
...
int main(int argc, char *argv[])
{
	...
	qb_log_init("cfsd", LOG_DAEMON, LOG_DEBUG);
	...
	for (int i = 10; i >= 0; i--) {
		if (flock(lockfd, LOCK_EX|LOCK_NB) != 0) {
			if (!i) {
				cfs_critical("unable to aquire cfsd lock: %s", strerror (errno));
				goto err;
			}
			if (i == 10)
				cfs_message("unable to aquire cfsd lock - trying again");
 ...
 ret:
	...
	cfs_message("exit cluster filesystem (%d)", ret);
